 ...
 *
 * $Id: heap.h,v 1.42 2003/05/13 16:24:13 gray Exp $
 */
...
#define IS_IN_HEAP(pnt)		\
  ((char *)(pnt) >= (char *)_dmalloc_heap_base \
   && (char *)(pnt) < (char *)_dmalloc_heap_last)

/* turn pointer PNT into a block index */
#define WHICH_BLOCK(pnt)	\
  (((char *)(pnt) - (char *)_dmalloc_heap_base) / BLOCK_SIZE)

/* get a pointer to the memory block number BLOCKN */
#define BLOCK_POINTER(block_n)	\
  ((char *)_dmalloc_heap_base + (block_n) * BLOCK_SIZE)

/* test whether pointer PNT is on a block boundary */
#define ON_BLOCK(pnt)		\
  (((char *)(pnt) - (char *)_dmalloc_heap_base) % BLOCK_SIZE == 0)

/* calculate the size of heap */
#define HEAP_SIZE	\
	((char *)_dmalloc_heap_last - (char *)_dmalloc_heap_base)
...
/* turn pointer PNT into a block index */
#define WHICH_BLOCK(pnt)		\
  (((char *)(_dmalloc_heap_base) - (char *)pnt) / BLOCK_SIZE)
...
#define ON_BLOCK(pnt)		\
  (((char *)(_dmalloc_heap_base) - (char *)(pnt)) % BLOCK_SIZE == 0)

/* is the heap growing?  is the NEW pointer ahead of the OLD */
#define HEAP_SIZE	\
   ((char *)_dmalloc_heap_base - (char *)_dmalloc_heap_last)
...
extern
void	*_dmalloc_heap_base;			/* base of our heap */

extern
void	*_dmalloc_heap_last;			/* end of our heap */
